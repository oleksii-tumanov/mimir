...
func TestParseLaneRequests(t *testing.T) {
	...
	cases := map[string]struct {
		...
		classes [][]compactorschedulerpb.CompactionClass
	}{
		"no lanes rejected": {
			input:   flagext.StringSliceCSV{},
			wantErr: true,
		...
		"compaction classes split across workers": {
			input: flagext.StringSliceCSV{"compact-fast+plan", "compact-slow"},
			workers: [][]compactorschedulerpb.JobType{
				{compaction, planning},
				{compaction},
			},
			classes: [][]compactorschedulerpb.CompactionClass{
				{compactorschedulerpb.COMPACTION_CLASS_FAST, compactorschedulerpb.COMPACTION_CLASS_ANY},
				{compactorschedulerpb.COMPACTION_CLASS_SLOW},
			},
		},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			for i, worker := range tc.classes {
				classes := make([]compactorschedulerpb.CompactionClass, len(result[i]))
				for j, r := range result[i] {
					classes[j] = r.CompactionClass
				}
