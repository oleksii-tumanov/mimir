...
func TestDeserializeCompactionJob_RecordWrittenBeforePlannerStats(t *testing.T) {
	...
	// Absent stats leave the job spanning nothing, so it lands in the fast lane.
	require.Zero(t, job.value.Duration())
	require.Equal(t, fastCompactionLane, newCompactionClassLanePolicy(testCompactionClassConfig()).LaneForJob(job))
}
