...
func TestJobTracker_ByteTracking(t *testing.T) {
	...
	// Spans 24h, so it lands in the slow lane.
	...
	bothPending := map[lane]splitMerge{fastCompactionLane: {split: 100}, slowCompactionLane: {merge: 200}}

	jt.recoverFrom([]*TrackedCompactionJob{splitJob, mergeJob}, nil)
	assertBytes("both jobs pending after recovery", bothPending)

	leaseResp, _, err := jt.Lease(fastCompactionLane)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assertBytes("split job complete", map[lane]splitMerge{slowCompactionLane: {merge: 200}})

	leaseResp, _, err = jt.Lease(slowCompactionLane)
	require.NoError(t, err)
	_, _, err = jt.Remove(leaseResp.Key.Id, leaseResp.Key.Epoch, true)
	require.NoError(t, err)
